...
class Conv2d(nn.Module):
    ...
    def forward(self, x):
        # print("**** GOES TO HERE lreq *****")
        # print("@"*80,"\n  (1) before inside lreq\n  x.shape:  {}\n".format(x.shape))
        if self.transpose:
            w = self.weight
            ...
            if not self.implicit_lreq:
                ...
                # print("@"*80,"\n (2) before inside lreq\n  x.shape:  {}  \n  w.shape:  {}\n".format(x.shape,w.shape))
                # if len(x.shape)==5:
                #     x = x.view(x.shape[1],x.shape[2],x.shape[3],x.shape[4])
                #     print("@"*80,"\n (2) before inside lreq\n  x.shape:  {}  \n".format(x.shape))

                # res = F.conv2d(x.view(x.shape[1],x.shape[2],x.shape[3],x.shape[4]), w, self.bias, stride=self.stride, padding=self.padding,
                #                 dilation=self.dilation, groups=self.groups)
                # print("@"*80,"\n (3) after inside lreq\n  x.shape:  {}  \n  w.shape:  {}  \n  res.shape:{}\n".format(x.shape,w.shape,res.shape))
                # return res#F.conv2d(x, w, self.bias, stride=self.stride, padding=self.padding,
                              # dilation=self.dilation, groups=self.groups)
