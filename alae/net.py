...
class EncoderDefault(nn.Module):
    def __init__(self, startf, maxf, layer_count, latent_size, channels=3):
        ...
        # print("from inside constructor of EncoderDefault => self.layer_count: {}".format(self.layer_count))
        for i in range(self.layer_count):
            ...
            mul *= 2
        # print(self.from_rgb)
        # print(self.encode_block)

    def encode(self, x, lod):
        
        styles = torch.zeros(x.shape[0], 1, self.latent_size)
        # styles = styles.to("cuda:0")# I added this to send this to cuda
        x = self.from_rgb[self.layer_count - lod - 1](x)
        # print("y from from_rgb: {}".format(x[0,0,0,0]))
        x = F.leaky_relu(x, 0.2)

        for i in range(self.layer_count - lod - 1, self.layer_count):
            # self.encode_block=self.encode_block.to("cuda:0")
